%% ExploreSimMoments
% Script to run `SimulatedMoments` with custom transition options and
% visualize the resulting time-series of migrant outcomes.
%
% The script performs the following steps:
%   1. Adds the `core` and `utils` folders to the MATLAB path.
%   2. Defines runtime options for the equilibrium solver, including
%      transition paths for $A_1$ and $B_1$.
%   3. Shows how to override structural parameters without editing
%      `SetParameters.m`.
%   4. Calls `SimulatedMoments` and produces diagnostic plots describing the
%      simulated economy.
%
% Figures produced:
...
%% Run full equilibrium simulation -------------------------------------------
out = SimulatedMoments(paramOverrides, opt);
...
if isfield(out.params, 'A_timePath') && ~isempty(out.params.A_timePath) && ...
    ...
    colIdx(colIdx > size(out.params.A_timePath, 2)) = size(out.params.A_timePath, 2);
